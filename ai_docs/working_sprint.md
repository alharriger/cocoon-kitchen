...
- **Phase 6 Task 3.5 (penalty-sensitive composite) is DONE — committed on `phase-6/evals`, retro complete.** Option 1 worst-dimension pull, α=0.4 (human-owned `composition.alpha` in `rubric.yaml`). Big win: band acc **39.2%→59.6%**, MAE **10.96→7.33**, Processed **1/12→7/12**, leniency +10.3→+2.2. Confirmed the residual was *composition* (aggregation), not sub-score leniency — a weighted mean structurally can't express "worst axis caps cleanliness." **Do NOT reopen this.** Branch NOT merged (Phase 6 incomplete).
- **THE NEXT BUILD is Task 4 — the bargain-model bake-off.** Run the tuned rubric across GLM/Gemini/Groq/DeepSeek/Qwen via `evaluate.py --model` and recommend an eval-selected default (never by brand). **BLOCKED on Amber's API keys** for the non-GLM providers (Gemini/Groq/DeepSeek/Qwen) — GLM is the only key in `.env` today. Confirm keys with Amber before starting; if unavailable, Task 5 (regression tracking + docs) can go first. Mind the bake-off models' own reasoning modes need thinking disabled like GLM (2026-07-11 pitfall) — leave `LLM_EXTRA_BODY` blank for models without a toggle and rely on validate-retry-once.
