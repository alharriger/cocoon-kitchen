...
- **THE NEXT BUILD is Task 6 — merge gates + merge to main** (Amber's manual test of Task 5 + full-phase gates first), then Task 7 (retro → pitfalls → refresh this doc for Phase 5 Deploy & Harden). **Amber's Task-5 manual test:** run `.venv/bin/python evals/evaluate.py` once, eyeball the appended `evals/run_log.csv` row + the printed "Baseline check" (should read within-noise), and decide whether to re-seed the baseline on a 52/52 pass.
- **Retro (Task 5):** one new pitfall logged (2026-07-20): a "read-only" review subagent executed the live eval harness and appended a stray row to the tracked run log (trimmed before commit) — subagent prompts must forbid live model calls, and tracked eval artifacts get a pre-commit inspection now. Task-5-specific: per-task security review of committed work meant the branch-wide gate re-covered old ground — fine, it stayed clean.
