...
| 5 | Regression tracking + docs | ⬜ | Persist each run's headline numbers + rubric version; keep `llm_contracts.md`/architecture/product in sync. Sets up Phase 7's "no regression" gate. **The bake-off already writes per-model result CSVs to `evals/results/` + logs headline numbers in the change-log — Task 5 formalizes a run-log/regression baseline on top.** |
...
- **THE NEXT BUILD is Task 5 — regression tracking + docs.** Persist each run's headline numbers + rubric version as a run-log/regression baseline (the bake-off already drops per-model CSVs in `evals/results/` and logs headline numbers in the change-log — Task 5 formalizes a baseline + a "no regression" gate for Phase 7). No new API keys needed. Then Task 6 (merge gates + merge to main) and Task 7 (retro → refresh this doc for Phase 5).
