...
- **Development default (build + prove the whole pipeline): Zhipu GLM-4.5-Flash on z.ai** — always $0 in/out, JSON-schema output, 128K context, OpenAI-compatible (`base_url=https://api.z.ai/api/openai/v1`, `model=glm-4.5-flash`), no card. Sign up on **z.ai international**, NOT bigmodel.cn (needs a Chinese phone). ~1 req/sec limit is fine for dev; validate-and-retry-once covers its weaker adversarial-JSON robustness. **Per Amber (2026-07-11): build and test the entire system on GLM-Flash first, then defer model choice to the eval bake-off.**
