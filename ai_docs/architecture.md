...
### 2026-07-18 — Phase 6 Task 3: tiered lexicons + calibration; composite formula under review
...
- **Open decision — composite composition formula (Amber, 2026-07-18):** `compose_score` is a **pure weighted mean** (a convex combination), so a single severe offender — Amber's example: chicken + broccoli + *artificial flavor* — gets averaged away (esp. `additive_count`, weight 0.05) and can't visibly drop the band. This is the mechanical cause of the residual "Processed → Mostly Clean" compression. **Proposal (to build next, pending Amber's pick):** make the composite penalty-sensitive so the *worst axis* visibly caps cleanliness. Options in `working_sprint.md`; recommended **worst-dimension pull** `composite = (1-α)·weighted_mean + α·min_subscore`, α human-owned in `rubric.yaml` — uses the model's existing sub-scores, adds no ingredient-parsing layer, keeps "composite is code-computed from human-owned knobs" (2026-07-09/11). **Revisit when:** Amber picks an option; measure before/after on the golden set. Do NOT chase this with more prompt strictness.
