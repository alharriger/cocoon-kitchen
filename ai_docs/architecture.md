...
- **Harness correctness (matters):** a naïve recommendation picked Groq at "77%" — but on only **13/52** rows a free-tier cap let through. Added a **coverage guard**: a run under 90% of rows is flagged and **excluded from the recommendation** (a rate-capped non-random slice is not comparable), plus per-provider **preflight**, per-row API-error skip, and free-tier **pacing**. No silent caps — excluded providers are named with their coverage.
- **Bake-off result (52-row golden, tuned rubric α=0.4), band / MAE / Processed:** `gpt-4o-mini` **63.5% / 6.77 / 11-12** · `gemini-2.5-flash-lite` 63.5% / 7.54 / 5-12 · **`glm-4.5-flash` (incumbent) 61.5% / 7.10 / 7-12** · `llama-3.3-70b` 57.7% / 8.50 / 8-12 · `mistral-small-3.2` 55.8% · `qwen-plus` 46.2% · `gpt-oss-120b` 46.0% (forced reasoning → slowest 37.5s, vindicates the thinking-model pitfall) · `glm-4.7-flash` 38.5% / MAE 22.77 (miscalibrated). `deepseek-chat` excluded (34/52, empty-JSON from the OR backend). **Eval-selected leader = `gpt-4o-mini`** (best on the Phase-6 target: Processed **11/12** vs baseline 1/12; slightly harsh on Clean 5/8).
- **Decision (Amber, 2026-07-19): keep `glm-4.5-flash` as the dev/prod default for now** — free on z.ai, balanced, a close 2nd — and re-run the OpenRouter bake-off at **periodic check-ins** now that it's wired, rather than switch to a paid model yet. Model/provider stays **eval-selected, never by brand** (2026-07-11). Spend: ~$0.36.

### 2026-07-18 — Phase 6 Task 3: tiered lexicons + calibration; composite formula RESOLVED (Task 3.5)
Built the marker lexicons as a console-owned `rubric/lexicons.yaml` (merged into the rubric at load) with **three flat** lists (nova4 / additives ↓, whole-food ↑) and **three 1–5 quality ladders** (`added_sugar_markers`, `fat_quality_markers`, `sodium_preservative_markers`) — each tier carries a target sub-score (worst → 0). Console gained a **Lexicons** tab (per-tier editors) so Amber curates the human-owned lists in place.
- **Measured finding (matters):** *populating the lists did nothing on its own* (band acc 32.7%, no better than baseline). What moved the number was the **prompt calibration rule** (scan-for-markers, "score by worst tier present", ingredient **decomposition**, anti-leniency nudge) **plus the tiers**: uncurated draft hit **44.2% / MAE 10.65**, post-curation **~40% / MAE ~11** (from ~32% / ~14). So the lever was *prompt grounding + calibration*, not the raw vocabulary. Passive reference lists are insufficient for a cheap model.
- **RESOLVED (Task 3.5, 2026-07-18) — composite composition formula.** `compose_score` was a **pure weighted mean** (a convex combination), so a single severe offender — Amber's example: chicken + broccoli + *artificial flavor* — got averaged away (esp. `additive_count`, weight 0.05) and couldn't drop the band, the mechanical cause of the residual "Processed → Mostly Clean" compression. **Decision (Amber, after a spectrum walk-through):** Option 1 **worst-dimension pull** — `composite = round((1-α)·weighted_mean + α·min_subscore)`, **α=0.4**, human-owned in `rubric.yaml` (`composition.alpha`, clamped [0,1], α=0 ≡ old mean). Chosen over threshold-penalty (two knobs that fight — undershot the low-weight spike yet nuked the bottom end) and power-mean (still weighted → the 0.05-weight offender barely moves). α acts on `min()` precisely *because* it's weight-independent. Uses the model's existing six sub-scores, **adds no ingredient-parsing layer**, keeps "composite is code-computed from human-owned knobs" (2026-07-09/11). **Measured (52-row golden, GLM-4.5-Flash):** band accuracy **39.2% → 59.6%**, MAE **10.96 → 7.33**, **Processed 1/12 → 7/12**, mean signed error **+10.3 → +2.2**; sub-score means flat (gain is composition, not drift); small predicted collateral (Clean 7/7 → 6/8). This **subsumed** the band-compression fix. Confirmed: don't chase leniency with more prompt strictness — the lever was the roll-up.
